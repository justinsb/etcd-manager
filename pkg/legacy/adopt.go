	...
	for _, k := range etcdKeys {
		...
		for _, v := range strings.Split(etcdInitialCluster, ",") {
			...
			for _, peerURL := range node.PeerUrls {
				...
				clientURL := strings.TrimSuffix(peerURL, peerSuffix) + clientSuffix
				quarantineURL := strings.TrimSuffix(peerURL, peerSuffix) + quarantineSuffix
			...
			if node.Name == etcdName {
				node.Name = etcdNodeConfiguration.Name
				node.ClientUrls = etcdNodeConfiguration.ClientUrls
				node.QuarantinedClientUrls = etcdNodeConfiguration.QuarantinedClientUrls
				node.PeerUrls = etcdNodeConfiguration.PeerUrls
			}
